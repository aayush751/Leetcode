class Solution {
public:
    unordered_map<int, vector<int>> prime2index;//first array
    unordered_map<int, vector<int>> index2prime;//reverse mapping 
    void dfs(int index, vector<int>& visitedIndex, unordered_map<int,bool>& visitedPrime){
        if(visitedIndex[index] == true) return;
        visitedIndex[index] = true;//else update visited index
        for(auto &prime : index2prime[index]){
            if(visitedPrime[prime] == true) 
                continue;
            visitedPrime[prime] = true;//visited prime array to be updated
    ...
    bool canTraverseAllPairs(vector<int>& nums) {
        for (int i=0; i<nums.size(); i++) {
            ...
            for (int j = 2; j*j <= nums[i]; j++) {
                if (temp % j == 0) {
                    prime2index[j].push_back(i);//j to i
                    index2prime[i].push_back(j);//i to j
                    while (temp % j == 0)
                      temp /= j;
                }
            }
